 ...
 * @author jmforhan
 */
public abstract class AbstractSharedHomeTestCaseJU5 {
	 ...
	 */
	protected String[] getYamlConfigFileNames() {
		return new String[] { "./configuration-test.yaml", };
	}
	...
	protected NodeConfig buildNodeConfig() {
		...
		final String[] yamlConfigs;
		final YamlAppConfigBuilder config = new YamlAppConfigBuilder(prop);
		final String value = prop.getProperty(BOOT_KEY);
		if (StringUtil.isEmpty(value)) {
			yamlConfigs = getYamlConfigFileNames();
		} else {
			yamlConfigs = prop.getProperty(BOOT_KEY).split(";");
			prop.remove(BOOT_KEY);
		}

		// si présent on récupère le paramétrage du fichier externe de
		// paramétrage log4j
		return config.withFiles(getClass(), yamlConfigs).build();
	}
