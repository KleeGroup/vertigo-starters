...
public final class ApplicationUser implements Entity {
	...
	public lollipop.domain.users.Profil getProfil() {
		...
		if (profil == null) {
			profil = io.vertigo.app.Home.getApp().getComponentSpace().resolve(io.vertigo.dynamo.store.StoreManager.class).getDataStore().read(fkURI);
		}
		return profil;
	}
	...
	public io.vertigo.dynamo.domain.model.URI<lollipop.domain.users.Profil> getProfilURI() {
		return io.vertigo.dynamo.domain.util.DtObjectUtil.createURI(this, "A_USR_PRO", lollipop.domain.users.Profil.class);
	}
